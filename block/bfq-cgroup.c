						...
						struct blkcg *blkcg)
{
	...
	for (queue_idx = 0; queue_idx < BFQ_NUM_ACTUATORS; queue_idx++) {
		async_bfqq = bic_to_bfqq(bic, 0, queue_idx);
		sync_bfqq = bic_to_bfqq(bic, 1, queue_idx);
		...
		if (async_bfqq) {
			...
			if (entity->sched_data != &bfqg->sched_data) {
				bic_set_bfqq(bic, NULL, 0, queue_idx);
